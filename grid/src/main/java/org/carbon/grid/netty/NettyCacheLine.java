...
class NettyCacheLine {
    private final long id;
    private volatile NettyCacheLineState state;
    private volatile int version;
    private short owner = -1;
    private NonBlockingHashSet<Short> sharers;
    private byte flags;
    private ByteBuf data;
    private final AtomicBoolean lockAvailable = new AtomicBoolean(true);
    private PriorityBlockingQueue<NettyMessage> messageBacklog;

    NettyCacheLine(long id, int version, short owner, ByteBuf data) {
        this.id = id;
    ...
    void lock(NettyMessage msg) {
        if (!lockAvailable.compareAndSet(true, false)) {
            // queue up
            synchronized (lockAvailable) {
                if (lockAvailable.get() && messageBacklog == null) {
                    messageBacklog = new PriorityBlockingQueue<>(13, Comparator.comparingInt(NettyMessage::getMessageSequenceNumber));
                }
            }
